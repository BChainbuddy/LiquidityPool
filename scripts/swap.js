...
async function swapTokens() {
    ...
    const token1 = await ethers.getContract("SimpleToken", deployer)
    const token2 = await ethers.getContract("SampleToken", deployer)
    ...
    const approveTx = await token1.approve(swapRouter.target, ethers.parseEther("1"))
    await approveTx.wait(1)
    console.log(`This is the deployer token1 balance before ${await token1.balanceOf(deployer)}`)
    console.log(`This is the deployer token2 balance before ${await token2.balanceOf(deployer)}`)
    console.log(
    ...
    const tx = await swapRouter.swapAsset(token1.target, token2.target, ethers.parseEther("1"), {
        value: getFee,
    })
    await tx.wait(1)
    ...
    console.log(`This is the deployer token1 balance after ${await token1.balanceOf(deployer)}`)
    console.log(`This is the deployer token2 balance after ${await token2.balanceOf(deployer)}`)
}
