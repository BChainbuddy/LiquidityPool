...
contract SwapRouter {
    ...
    function swapAsset(
        ...
        if (poolTracker.exists(address1, address2)) {
            ...
            if (pool1.assetOneAddress() == address1) {
                routingTokenAmount = pool1.sellAssetOne{value: pool1.swapFee()}(inputAmount);
            } else {
                routingTokenAmount = pool1.sellAssetTwo{value: pool1.swapFee()}(inputAmount);
            }
            ...
            if (pool2.assetOneAddress() == routingToken) {
                amountOutput = pool2.sellAssetOne{value: pool2.swapFee()}(routingTokenAmount);
            } else {
                amountOutput = pool2.sellAssetTwo{value: pool2.swapFee()}(routingTokenAmount);
            }
            IERC20(address2).transfer(msg.sender, amountOutput);
            // Unrequired fee
     ...
     */
    function getSwapFee(address address1, address address2) public view returns (uint256) {
        uint256 fee;
        if (poolTracker.exists(address1, address2)) {
            LiquidityPool pool = poolTracker.pairToPool(address1, address2);
            fee += pool.swapFee();
        ...
        for (uint256 i; i < token1pairs.length; i++) {
            for (uint256 a; a < token2pairs.length; a++) {
                if (token1pairs[i] == token2pairs[a]) {
                    for (uint256 b; b < poolTracker.getRoutingAddressesLength(); b++) {
                        (address tokenAddress, address priceFeed) = poolTracker.routingAddresses(b);
                        if (tokenAddress == token1pairs[i]) {
                            int liquidity;
                            LiquidityPool pool1 = poolTracker.pairToPool(address1, tokenAddress);
                            LiquidityPool pool2 = poolTracker.pairToPool(address2, tokenAddress);
                            uint256 balance1 = IERC20(tokenAddress).balanceOf(address(pool1));
                            uint256 balance2 = IERC20(tokenAddress).balanceOf(address(pool2));
                            liquidity = (int(balance1) + int(balance2)) * usdConverter(priceFeed);
